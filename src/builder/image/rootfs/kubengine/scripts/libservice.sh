...
. /opt/kubengine/scripts/libvalidations.sh
...
generate_systemd_conf() {
    ...
    local -r service_file="${systemd_units_dir}/kubengine.${service_name}.service"
    # Default values
...
[Unit]
Description=kubengine service for ${name}
# Starting/stopping the main kubengine service should cause the same effect for this service
PartOf=kubengine.service
...
[Install]
# Enabling/disabling the main kubengine service should cause the same effect for this service
WantedBy=kubengine.service
EOF
}
