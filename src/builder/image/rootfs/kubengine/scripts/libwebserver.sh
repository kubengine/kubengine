...
web_server_execute() {
    ...
    (
        . "/opt/kubengine/scripts/lib${web_server}.sh"
        . "/opt/kubengine/scripts/${web_server}-env.sh"
        "$@"
    )
}
...
web_server_list() {
    ...
    for web_server in "${supported_web_servers[@]}"; do
        [[ -f "/opt/kubengine/scripts/${web_server}-env.sh" ]] && existing_web_servers+=("$web_server")
    done
...
web_server_validate() {
    ...
    if [[ -z "$(web_server_type)" || ! " ${supported_web_servers[*]} " == *" $(web_server_type) "* ]]; then
        ...
        print_validation_error "Could not load the $(web_server_type) web server library from /opt/kubengine/scripts. Check that it exists and is readable."
    fi
...
web_server_start() {
    ...
    if [[ "${KUBENGINE_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl start "kubengine.$(web_server_type).service"
    else
        "${KUBENGINE_ROOT_DIR}/scripts/$(web_server_type)/start.sh"
    fi
}
...
web_server_stop() {
    ...
    if [[ "${KUBENGINE_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl stop "kubengine.$(web_server_type).service"
    else
        "${KUBENGINE_ROOT_DIR}/scripts/$(web_server_type)/stop.sh"
    fi
}
